...
class FunctionExporter(object):
    ...
    def _write_cwrapper(self):
        ...
        with open(os.path.join(self.workdir, 'wrapper.c'), 'w') as c:
            ...
            c.write('#define MAX_PATH 1024\n')
            c.write('#ifndef _WIN32\n')
            c.write('    char *workdir = mkdtemp("/tmp/otfmi_XXXXXX");\n')
            c.write('#else\n')
            c.write('    const char * tempEnv = getenv("TEMP");\n')
            c.write('    char workdir[MAX_PATH];\n')
            c.write('    int ret = GetTempFileName(tempEnv, "abc", 0, workdir)\n')
            c.write('#endif\n')
            c.write('    if (access(workdir, R_OK) == -1)\n#ifdef _WIN32\n      _mkdir(workdir);\n#else\n      mkdir(workdir, 0733);\n#endif\n')
            c.write('    char apath[MAX_PATH] = {0};\n')
            c.write('#ifndef _WIN32\n')
            c.write('    const char path_sep = \'/\';\n')
            c.write('#else\n')
            c.write('    const char path_sep = \'\\\\\';\n')
            c.write('#endif\n')
            c.write('    sprintf(apath, "%s%cpoint.in", workdir, path_sep);\n')
            c.write('    fptr = fopen(apath, "w");\n')
            c.write('    for (i = 0; i < nin; ++ i)\n')
            c.write('      fprintf(fptr, "%lf\\n", x[i]);\n')
            c.write('    fclose(fptr);\n')
            c.write('    char xml_path[MAX_PATH] = {0};\n')
            c.write('    sprintf(xml_path, "%s%cfunction.xml", workdir, path_sep);\n')
            c.write('    fptr = fopen(xml_path, "wb");\n')
            c.write('    fwrite (xml_path, sizeof(char), sizeof(xml_data), fptr);\n')
            c.write('    fclose(fptr);\n')
            c.write('    char in_path[MAX_PATH] = {0};\n')
            c.write('    sprintf(in_path, "%s%cpoint.in", workdir, path_sep);\n')
            c.write('    char out_path[MAX_PATH] = {0};\n')
            c.write('    sprintf(out_path, "%s%cpoint.out", workdir, path_sep);\n')
            c.write('    char py_path[MAX_PATH] = {0};\n')
            c.write('    sprintf(py_path, "%s%cwrapper.py", workdir, path_sep);\n')
            ...
            c.write('      fprintf(fptr, "with open(\\"%s\\", \\"r\\") as f:\\n", in_path);\n')
            ...
            c.write('      fprintf(fptr, "with open(\\"%s\\", \\"w\\") as f:\\n", out_path);\n')
            c.write('      fprintf(fptr, "    for v in y:\\n");\n')
            c.write('      fprintf(fptr, "        f.write(str(v)+\\"\\\\n\\")\\n");\n')
            c.write('      fclose(fptr);\n')
            c.write('    char cmd_str[MAX_PATH] = {0};\n')
            c.write('    char log_path[MAX_PATH] = {0};\n')
            c.write('    sprintf(log_path, "%s%cerror.log", workdir, path_sep);\n')
            c.write('    sprintf(cmd_str, "python %s > %s 2>&1", py_path, log_path);\n')
            c.write('    if (rc != 0) printf("otfmi: error running \\"%s\\" rc=%d\\n", cmd_str, rc);\n')
            c.write('    fptr = fopen(out_path, "r");\n')
